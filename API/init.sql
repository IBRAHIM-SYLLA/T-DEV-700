...
-- 5. Création de la table WORK_SCHEDULES (Planning spécifique à une date)
CREATE TABLE IF NOT EXISTS work_schedules (
    schedule_id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    schedule_date DATE NOT NULL, -- La date du planning, doit être NOT NULL
    ...
    UNIQUE KEY unique_user_schedule_date (user_id, schedule_date) -- Contrainte unique corrigée
);
...
-- Mise à jour de l'index pour schedule_date
CREATE INDEX idx_schedules_user_date ON work_schedules(user_id, schedule_date);
...
SELECT 
    ...
    u.last_name, 
    t.name as team_name,
    DATE(c.arrival_time) as work_date, -- Regroupe par la date du jour
    w.expected_arrival_time,
    w.expected_departure_time,
    w.schedule_date, -- Ajout de la date de planning spécifique
    ...
    TIMESTAMPDIFF(MINUTE, w.expected_arrival_time, TIME(MIN(c.arrival_time))) as retard_minut
FROM clocks c 
JOIN users u ON c.user_id = u.user_id 
JOIN teams t ON t.team_id = u.team_id 
-- Jointure directe sur la date du planning
JOIN work_schedules w ON w.user_id = c.user_id 
    AND DATE(c.arrival_time) = w.schedule_date -- La jointure utilise maintenant la date complète
